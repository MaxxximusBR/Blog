...
type Origin = 'official' | 'robot' | 'mixed';
...
type NewsItem = {
  ...
  source?: string;
  origin?: Origin;         // <<< novo: proveniência
};
...
async function loadNews(): Promise<NewsItem[]> {
  ...
  // 2) mistura IA: enriquece quando já existe oficial
  for (const ai of robot) {
    if (byUrl.has(ai.url)) {
      ...
      byUrl.set(ai.url, {
        ...
        origin: 'mixed', // <<< promovido + enriquecido
      });
    } else {
      byUrl.set(ai.url, ai); // só IA
    }
  }
      ...
      {items.length === 0 ? (
        ...
        <section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((n) => {
            ...
            const isIA = n.origin === 'robot' || n.origin === 'mixed';
            ...
            return (
              ...
              >
                ...
                <div className="text-xs opacity-70 flex items-center gap-2">
                  ...
                  {isIA && (
                    <span className="ml-1 rounded border border-emerald-400/30 bg-emerald-500/15 px-2 py-0.5 text-emerald-300">
                      IA
                    </span>
                  )}
                  {scoreTxt && (
