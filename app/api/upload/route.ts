...
export async function POST(req: Request) {
  try {
    ...
    if (!ct.includes('multipart/form-data')) {
      return NextResponse.json({ ok:false, msg:'Envie como multipart/form-data (FormData).' }, { status: 400 });
    }
    ...
    if (bytes.byteLength > 4.5 * 1024 * 1024) {
      throw new Error('PDF acima de ~4,5 MB nesta rota. Posso habilitar upload direto do navegador → Blob para maiores.');
    }
    ...
    if (idxURL) {
      try { const r = await fetch(idxURL, { cache: 'no-store' }); if (r.ok) index = await r.json(); } catch {}
    }
