...
export default function Dashboard() {
  ...
  const monthData: CountryData[] = useMemo(()=> Object.entries(map).map(([code, count])=>({ code, name: codeToName[code] || code, count: Number(count) })).sort((a,b)=>b.count-a.count), [map]);
  const maxValue = useMemo(()=> Math.max(1, ...Object.values(map), 1), [map]);
  const scale = useMemo(()=> (scaleQuantize<number, string>() as any).domain([0, maxValue]).range(['#e0e7ff','#c7d2fe','#a5b4fc','#818cf8','#6366f1','#4f46e5']), [maxValue]);
  ...
  return (
    <div className="space-y-8">
      <section className="card">
        ...
        <p className="hint">Passe o mouse para ver valores; clique para fixar um país. Lista de fallback ao lado.</p>
      ...
      <section className="grid xl:grid-cols-3 gap-6">
        <div className="card xl:col-span-2">
          <div className="flex items-center justify-between mb-3 gap-4">
            ...
            <div className="flex items-center gap-2">
              <input placeholder="Código do país (ex.: BRA)" className="bg-black/40 border border-gray-700 rounded px-3 py-1 w-56" value={selected} onChange={(e)=> setSelected(e.target.value.toUpperCase()) } />
          ...
          <div className="relative">
                  ...
                  {({geographies})=> geographies.map((g)=>{
                    ...
                    return(
                      <Geography key={g.rsmKey} geography={g}
                        onMouseEnter={(e)=> setTip({ text: `${codeToName[code] || code || '—'} (${code||'?'}) — ${value} casos em ${month}`, x:e.clientX, y:e.clientY })}
                        onMouseMove={(e)=> setTip({ text: `${codeToName[code] || code || '—'} (${code||'?'}) — ${value} casos em ${month}`, x:e.clientX, y:e.clientY })}
            ...
            {tip && (<div className="pointer-events-none fixed z-50 px-2 py-1 rounded bg-black/80 border border-gray-700 text-xs" style={{ left: tip.x+12, top: tip.y+12 }}>{tip.text}</div>)}
          </div>

          <div className="mt-4">
            <div className="text-xs text-gray-400 mb-1">Legenda — intensidade (0 → {maxValue})</div>
            <div className="flex items-center gap-2">{['#e0e7ff','#c7d2fe','#a5b4fc','#818cf8','#6366f1','#4f46e5'].map((c,i)=> <div key={i} className="h-3 w-10 rounded" style={{background:c}} />)}</div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            <div className="text-xl font-semibold">{(codeToName[selected] || selected || '—')} {selected && <span className="text-gray-500">({selected})</span>}</div>
          ...
          <ul className="text-sm max-h-60 overflow-auto divide-y divide-gray-800">
            {monthData.map(row => (
              <li key={row.code} className={`flex items-center justify-between py-1 ${selected===row.code?'text-indigo-300':''}`}>
                <button className="text-left hover:underline" onClick={()=> setSelected(row.code)}>{row.name} <span className="text-gray-500">({row.code})</span></button>
