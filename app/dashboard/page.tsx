...
function prevOf(monthKey: string) {
  ...
  const d = new Date(y, (m || 1) - 2, 1); // -1 mês (0..11)
  const yy = d.getFullYear();
  const mm = String(d.getMonth() + 1).padStart(2, '0');
  return `${yy}-${mm}`;
}

// ======================= Mini-componentes do rodapé =======================
...
function MonthReports({ monthKey }: { monthKey: string }) {
  ...
  React.useEffect(() => {
    ...
    fetch('/api/reports', { cache: 'no-store' })
      .then((r) => r.json())
      .then((all) => {
        if (ok) setItems(Array.isArray(all) ? all : []);
      })
      .catch(() => {});
    return () => {
      ok = false;
        ...
        {list.map((r: any) => (
          <li key={r.slug} className="flex items-center justify-between gap-2">
            <div>
              ...
              {r.summary && (
                <div className="text-sm opacity-80 line-clamp-2">{r.summary}</div>
              )}
            </div>
            <div className="shrink-0 flex gap-2">
              <a href={`/report/${r.slug}`} className="btn">
                Abrir
              </a>
              <a href={r.file} target="_blank" rel="noopener" className="btn">
                PDF
              </a>
            </div>
          </li>
...
function MonthDelta({
  ...
  const diffs = Array.from(union).map((k) => ({
    iso3: k,
    diff: (cur[k] || 0) - (prev[k] || 0),
    now: cur[k] || 0,
  }));
  const up = diffs
    .filter((d) => d.diff > 0)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <ul className="space-y-1">
            {up.length === 0 ? (
              <li className="opacity-70 text-sm">—</li>
            ) : (
              up.map((x) => (
                <li key={x.iso3} className="flex justify-between">
                  ...
                  <span className="tabular-nums text-emerald-400">
                    +{x.diff}{' '}
                    <span className="opacity-70">({x.now})</span>
                  </span>
                </li>
              ))
            )}
...
export default function Dashboard() {
  ...
  const [month, setMonth] = useState<string>(
    normalizeMonth((aggregates as any).defaultMonth)
  );
  ...
  const maxValue = useMemo(
    () => Math.max(1, ...Object.values(map), 1 as any),
    [map]
  );
  const scale = useMemo(
    ...
    <main className="max-w-7xl mx-auto p-6 space-y-8">
      ...
      <section className="relative rounded-2xl bg-[#0e1624] px-6 py-5 shadow-lg overflow-hidden">
        <h1 className="text-2xl font-semibold">
          Consolidação Global —{' '}
          <span className="text-xs align-middle">Dashboard V9b (OFFLINE+secure)</span>
        </h1>
        <p className="hint">
          Passe o mouse para ver valores; clique para fixar um país. A busca de
          siglas está ao lado.
        </p>
                      ...
                      return (
                        <Geography
                          ...
                          style={{
                            default: {
                              fill,
                              outline: isSel ? MAP_OUTLINE_HL : MAP_OUTLINE,
        ...
        <aside className="card">
          ...
          <div className="space-y-3">
            <div className="text-xl font-semibold">
              ...
              {selected && (
                <span className="text-gray-500">({normalizeCode(selected)})</span>
              )}
            </div>
