...
// --- decodificador simples p/ METAR em PT-BR (cobre campos + comuns)
function decodeMetar(m: any) {
  // Estrutura do AWC "format=json": propriedades típicas (schema atualizado em 2025).
  // Tentamos mapear de forma robusta.
  const raw = m?.rawOb ?? m?.raw_text ?? '';
  const icao = m?.icaoId ?? m?.station_id ?? m?.id ?? '????';
  ...
  // tempo presente (fenômenos)
  const wx = (m?.wx ?? m?.weather_string ?? m?.present_weather ?? '')
    .toString();
  ...
  const cloudsTxt = Array.isArray(clouds)
    ? clouds.map((c: any) => {
        ...
        const mapCov: Record<string,string> = {
          ...
          SCT: 'esparsas', BKN: 'fragmentadas', OVC: 'encoberto', OVX: 'visibilidade vertical'
        };
...
function useTicker(ms:number){
  ...
  return tick; // só p/ re-render de relógio
}
...
export default function MetarPage(){
  ...
  useTicker(1000); // mostra “agora” avançando

  async function loadAll(){
    try{
      setErr(null);
      // últimas 12h para cada estação (mantém histórico do dia)
      ...
      // AWC mudou schema em 2025 — tentamos achar a lista de registros de forma flexível
  ...
  const grouped = useMemo(() => {
    // organiza por ICAO mais recente primeiro
    ...
    for(const [k, arr] of map) arr.sort((a,b) => (new Date(b?.obsTime ?? b?.time ?? b?.issueTime ?? 0).getTime()) - (new Date(a?.obsTime ?? a?.time ?? a?.issueTime ?? 0).getTime()));
    return map;
  }, [metars]);

  return (
    <main className="relative">
      {/* BG super escuro para combinar com as outras páginas */}
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-6 md:py-10">
        <header className="mb-6 md:mb-8">
          ...
          <p className="mt-2 text-sm opacity-80">
            Fonte: <a className="underline" href="https://aviationweather.gov/data/api/" target="_blank">Aviation Weather Center / NOAA</a>. Atualiza automaticamente a cada 60s. :contentReference[oaicite:2]{index=2}
          </p>
        </header>
        ...
        <div className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-4">
          ...
          <p className="text-xs opacity-70 mb-3">
            Exibindo SIGMET(s) internacionais com FIR: SBAZ (Amazônica), SBBS (Brasília), SBRE (Recife), SBCW (Curitiba). Fonte: AWC/NOAA. :contentReference[oaicite:3]{index=3}
          </p>
        ...
        <footer className="mt-8 text-xs opacity-70">
          ...
          Restrições: sem CORS (uso via rotas servidor) e rate limit (~100 req/min). :contentReference[oaicite:4]{index=4}
        </footer>
      </section>
    </main>
  );
