...
function extractMetars(j: any): any[] {
  ...
  if (Array.isArray(j?.data?.METAR)) return j.data.METAR;   // ADDS oficial
  if (Array.isArray(j?.data?.metars)) return j.data.metars;
  if (Array.isArray(j?.features)) {
    // se vier em GeoJSON (improvável para METAR), tenta extrair properties
...
function decodeMetar(m: any) {
  const raw =
    m?.rawOb ?? m?.raw_text ?? m?.raw ?? m?.properties?.rawOb ?? '';
  ...
  const timeISO =
    ...
    m?.issueTime ??
    m?.observed ??
    m?.observation_time ?? // ADDS
    null;

  // vento
  const windDir =
    m?.wind?.dir ?? m?.wind_dir_degrees ?? m?.wdir ?? m?.winddir ?? null;
  const windSpd =
    m?.wind?.spd_kts ?? m?.wind_speed_kt ?? m?.wspd ?? m?.windspd ?? null;
  const windGst =
    m?.wind?.gust_kts ?? m?.wind_gust_kt ?? m?.wgst ?? m?.windgst ?? null;

  // visibilidade
  const visSm =
    m?.visibility?.sm ?? m?.visibility_statute_mi ?? m?.vis ?? null;

  // tempo presente
  const wx =
    (m?.wx ?? m?.wx_string ?? m?.weather_string ?? m?.present_weather ?? '')
      .toString();
  ...
  const alt =
    m?.altim?.in ?? m?.altim_in_hg ?? m?.altimeter?.in ?? m?.altim ?? null;
  ...
  const vis =
    visSm != null
      ? `${visSm} SM (~${(Number(visSm) * 1.609).toFixed(1)} km)`
      : '—';
...
export default function MetarPage() {
  const allIds = useMemo(
    () => STATIONS.flatMap((s) => s.icaos).join(','),
    []
  );
  ...
  async function loadAll() {
    try {
      ...
      const mRes = await fetch(
        `/api/awx/metar?ids=${encodeURIComponent(allIds)}&hours=12`,
        { cache: 'no-store' }
      );
  ...
  const grouped = useMemo(() => {
    ...
    for (const r of metars) {
      const icao = (r?.icaoId ?? r?.station_id ?? r?.id ?? r?.station ?? '')
        .toUpperCase();
      if (!icao) continue;
  ...
  return (
    <main className="relative">
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-6 md:py-10">
        ...
        {err && (
          <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-sm mb-4">
            Erro: {err}
          </div>
        )}
            ...
            >
              <div className="mb-3 flex items-center justify-between">
                ...
                <div className="text-xs opacity-70">
                  Esta&ccedil;&otilde;es: {icaos.join(', ')}
                </div>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-xs opacity-70">
                          {d?.timeISO
                            ? new Date(d.timeISO).toLocaleString()
                            : loading
                            ? 'carregando...'
                            : '—'}
                        </div>
                      ...
                      {d ? (
                        <>
                          <div className="mt-2 text-sm leading-relaxed">
                            ...
                            <div>
                              <span className="opacity-70">Visibilidade:</span>{' '}
                              {d.vis}
                            </div>
                            {d.wx && (
                              <div>
                            ...
                            {d.nublado && (
                              <div>
                                <span className="opacity-70">Nuvens:</span>{' '}
                                {d.nublado}
                              </div>
                            )}
                            <div className="mt-1 opacity-80 text-xs">
                              {d.temp != null && <>Temp {d.temp}°C</>}{' '}
                              {d.dew != null && <> • Pto Orv {d.dew}°C</>}
                              {d.alt != null && <> • Altímetro {d.alt} inHg</>}
                            </div>
                          </div>
                          <div className="mt-2 rounded bg-white/5 px-2 py-1 font-mono text-xs">
                            {d.raw}
                          </div>
                        </>
                      ) : (
                              ...
                              return (
                                ...
                                >
                                  <div className="text-[11px] opacity-70">
                                    {dd.timeISO
                                      ? new Date(dd.timeISO).toLocaleString()
                                      : ''}
                                  </div>
        ...
        <div className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-4">
          <h2 className="text-lg font-semibold mb-2">
            SIGMET — FIR Brasil (ativos)
          </h2>
          <p className="text-xs opacity-70 mb-3">
            Exibindo SIGMET(s) internacionais com FIR: SBAZ (Amazônica), SBBS
            (Brasília), SBRE (Recife), SBCW (Curitiba). Fonte: AWC/NOAA.
          </p>

          {!sigmet ||
          !Array.isArray(sigmet.features) ||
          sigmet.features.length === 0 ? (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="font-semibold">
                        {(p.hazard ?? 'SIGMET').toString().toUpperCase()} —{' '}
                        {p.fir ?? p.firId ?? p.firname ?? ''}
                      </div>
                      <div className="text-xs opacity-70">
                        {p.validTimeFrom} → {p.validTimeTo}
                      </div>
                    </div>
        ...
        <footer className="mt-8 text-xs opacity-70">
          ...
          . Restrições: sem CORS (uso via rotas servidor) e rate limit (~100
          req/min).
        </footer>
      </section>
    </main>
